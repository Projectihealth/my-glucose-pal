...
export function CGMTab({ cgmGoals, onAddCGMGoal }: CGMTabProps) {
  ...
  const handleAddToGoals = (patternId: string, actionId: string) => {
    ...
    const newGoal: CGMGoal = {
      id: `cgm-${Date.now()}`,
      title: action.title,
      description: action.description,
      target_count: action.targetCount,
      current_count: 0,
      category: action.category,
      icon: action.icon.name || 'Utensils',
      week: 'This Week',
    };

    onAddCGMGoal(newGoal);
