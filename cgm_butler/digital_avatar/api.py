...
def init_avatar_api(tavus_api_key: str = None, persona_id: str = None, openai_api_key: str = None):
    ...
    print("\n" + "="*60)
    print("🤖 初始化 Digital Avatar API...")
    print("="*60)
    
    # Tavus conversation manager (for video avatar) - optional, will fail gracefully
    try:
        api_key = tavus_api_key or os.getenv('TAVUS_API_KEY')
        if api_key:
            print(f"📡 Tavus API Key: {api_key[:20]}...")
        ...
        print(f"❌ Failed to initialize Tavus API: {e}")
    ...
    print("="*60)
    print(f"✅ Avatar API 初始化完成")
    print(f"   Video Chat: {'可用' if conversation_manager else '不可用'}")
    print(f"   Text Chat: {'可用' if gpt_chat_manager else '不可用'}")
    print("="*60 + "\n")
